...
ядрами**, которые и гоняют трафик: на десктопе `keqrnel` и `wireproxy`, на Android
`libxray.so` с `tun2socks`. Серверов приложение не предоставляет — пользователь
приносит свои подписки.
